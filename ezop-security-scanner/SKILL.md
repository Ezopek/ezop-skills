...
- `$ezop-pr-reviewer` for PR-scoped security as part of broader quality review
- `$ezop-cicd-guardian` for pipeline-specific security (supply chain, action permissions, secrets management)
- `$ezop-slice-reviewer` for slice acceptance with security focus
- `$ezop-repo-drift-auditor` for detecting security posture drift over time
...
> Using **ezop-security-scanner** to [specific purpose based on the request].
...
```text
$ezop-security-scanner przeprowadz pelny audyt bezpieczenstwa tego repo.
...
```text
$ezop-security-scanner perform a full security audit of this repository.
...
```text
$ezop-security-scanner scan dependencies for known vulnerabilities, outdated packages, and license risks.
...
```text
$ezop-security-scanner perform a pre-release security review.
...
```text
$ezop-security-scanner audit the authentication and authorization module for security vulnerabilities.
...
```text
$ezop-security-scanner audit this repo for security vulnerabilities
```
...
- the task is CI/CD pipeline audit (use `$ezop-cicd-guardian`)
- the task requires active penetration testing or dynamic analysis
- the task is a general code quality review without security focus (use `$ezop-pr-reviewer`)
...
- `$ezop-cicd-guardian` — for pipeline-specific security (supply chain, action permissions, secrets management)
- `$ezop-pr-reviewer` — for PR-scoped security review as part of broader quality check
- `$ezop-agent-ready-docs` — when security documentation or AGENTS.md security guidance needs updating
- `$ezop-repo-drift-auditor` — when security findings suggest documentation has drifted from actual security posture

## Relationship To Other Skills

Use this skill alongside `$ezop-pr-reviewer` when:
...
Use this skill alongside `$ezop-cicd-guardian` when:
...
Use this skill alongside `$ezop-slice-reviewer` when:
...
Use this skill alongside `$ezop-repo-drift-auditor` when:
...
Recommend `$ezop-agent-ready-docs` when:
...
Recommend `$ezop-delivery-planner` when:
...
Recommend `$ezop-backlog-loop-orchestrator` when:
