...
1. `$agent-ready-docs` fixes repo source-of-truth docs
2. `$delivery-planner` creates or refreshes roadmap and backlog artifacts
3. `$backlog-loop-orchestrator` executes bounded slices
4. `$slice-reviewer` checks one finished slice
5. `$repo-drift-auditor` periodically checks whether those artifacts still match reality

## Announce At Start

When this skill activates, tell the user:

> Using **repo-drift-auditor** to [specific purpose based on the request].
...
```text
$repo-drift-auditor sprawdz, czy dokumentacja, backlog, workflowy i realny stan repo nie rozjechaly sie ze soba.
Najpierw przeczytaj `AGENTS.md`, `README.md`, backlogi lub roadmapy, a potem zweryfikuj komendy, CI, kluczowe docs i aktualny kod.
Chce findings-first audit driftu oraz jasne wskazanie, czy kolejnym krokiem powinien byc `$agent-ready-docs`, `$delivery-planner`, `$backlog-loop-orchestrator` czy `$slice-reviewer`.
```

Recommended English invocation:

```text
$repo-drift-auditor audit whether the docs, backlog, workflows, and actual repo state have drifted apart.
Read `AGENTS.md`, `README.md`, the roadmap or backlog docs, and then verify commands, CI, key docs, and the current code.
I want a findings-first drift audit plus a clear recommendation for whether `$agent-ready-docs`, `$delivery-planner`, `$backlog-loop-orchestrator`, or `$slice-reviewer` should handle the next step.
```

Minimal invocation:

```text
$repo-drift-auditor audit this repo for docs and backlog drift
```
...
- `$agent-ready-docs` when the source-of-truth docs need repair
- `$delivery-planner` when roadmap or backlog artifacts need re-planning
- `$backlog-loop-orchestrator` when the artifacts are sound and the repo is ready for execution
- `$slice-reviewer` when the real issue is acceptance of one specific completed slice, not repo-wide drift
...
- `$cicd-guardian` — for deep CI/CD pipeline drift analysis beyond what a general audit covers
- `$security-scanner` — when drift findings suggest security posture may have degraded
- `$pr-reviewer` — when drift is localized to recent changes that need quality review
