...
- clear docs impact
- output that works well with $ezop-backlog-loop-orchestrator
...
Create or update a backlog that is meant to be executed later by $ezop-backlog-loop-orchestrator.
Read the repo instructions, the current docs, and the existing backlog or roadmap first.
If the documentation foundation is too weak or too stale to support reliable planning, say so and recommend running $ezop-agent-ready-docs first.
