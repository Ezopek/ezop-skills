...
Review the result of this worker-completed bounded slice for use inside $backlog-loop-orchestrator.
Check the implementation against the approved slice, then judge whether it is truly ready for commit.

If the slice definition is too weak, say so and recommend $delivery-planner.
If docs drift prevents trustworthy acceptance, say so and recommend $agent-ready-docs.
```
