...
This is a general-purpose review skill. It is different from `$ezop-slice-reviewer`, which reviews one bounded slice against its approved plan.
...
It works as a standalone quality gate for any PR, or as a final quality pass after `$ezop-backlog-loop-orchestrator` completes a batch of work.

This skill fits into the ezop workflow pipeline:

- `$ezop-agent-ready-docs` makes repo guidance trustworthy
- `$ezop-delivery-planner` produces bounded slices or feature plans
- `$ezop-backlog-loop-orchestrator` runs plan -> implement -> verify cycles
- `$ezop-slice-reviewer` checks each slice against its approved plan
- `$ezop-repo-drift-auditor` can periodically confirm docs and backlog still match reality
- `$ezop-pr-reviewer` performs a broad quality, security, and maintainability review before merge
- `$ezop-cicd-guardian` can validate pipeline health and deployment readiness
- `$ezop-security-scanner` can do deep security-focused analysis of changes

## Announce At Start

When this skill activates, tell the user:

> Using **ezop-pr-reviewer** to [specific purpose based on the request].
...
```text
$ezop-pr-reviewer zrob review tego PR-a pod katem jakosci kodu, bezpieczenstwa, wydajnosci, testow i utrzymywalnosci.
...
```text
$ezop-pr-reviewer review this pull request for code quality, security, performance, test coverage, and maintainability.
...
```text
$ezop-pr-reviewer review this PR
```

Paired invocation with security scanner:

```text
$ezop-pr-reviewer review this PR for quality and maintainability, then use $ezop-security-scanner for a deep security pass on the same diff.
```
...
- `$ezop-backlog-loop-orchestrator` has finished a batch and the result needs a broad quality check
...
- the task is plan compliance review for a bounded slice (use `$ezop-slice-reviewer`)
...
- `$ezop-security-scanner` — for deep security-focused review
- `$ezop-slice-reviewer` — when review should also check plan compliance

## Relationship To Other Skills

Use this skill instead of `$ezop-slice-reviewer` when:
...
Use `$ezop-slice-reviewer` instead of this skill when:
...
Recommend `$ezop-security-scanner` when:
...
Recommend `$ezop-cicd-guardian` when:
...
Recommend `$ezop-repo-drift-auditor` when:
