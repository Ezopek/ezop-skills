...
- `$ezop-agent-ready-docs` — document pipeline conventions and update docs after pipeline changes
- `$ezop-delivery-planner` — plan pipeline changes as bounded slices
- `$ezop-slice-reviewer` — review pipeline-related slices before commit
- `$ezop-repo-drift-auditor` — detect when pipeline definitions have drifted from documented CI/CD expectations
- `$ezop-security-scanner` — deep supply chain and dependency security analysis beyond pipeline scope

## Announce At Start

When this skill activates, tell the user:

> Using **ezop-cicd-guardian** to [specific purpose based on the request].
...
```text
$ezop-cicd-guardian zaudytuj pipeline'y CI/CD w tym repo pod katem bezpieczenstwa, poprawnosci i wydajnosci.
...
```text
$ezop-cicd-guardian audit the CI/CD pipelines in this repo for security, correctness, and performance.
...
```text
$ezop-cicd-guardian audit this repo's CI/CD pipelines
```

Creation invocation:

```text
$ezop-cicd-guardian create a GitHub Actions pipeline for this project with lint, test, build, and deploy stages.
...
```text
$ezop-cicd-guardian optimize the CI/CD pipeline build times in this repo.
...
```text
$ezop-cicd-guardian harden the CI/CD pipelines in this repo.
...
- the task is pure code review without pipeline involvement (use `$ezop-pr-reviewer`)
...
- `$ezop-agent-ready-docs` — when pipeline docs need updating after pipeline changes
- `$ezop-security-scanner` — for deep supply chain and dependency security beyond pipeline scope
- `$ezop-repo-drift-auditor` — when pipeline definitions have drifted from documented CI/CD expectations
- `$ezop-delivery-planner` — when pipeline changes should be planned as bounded slices

## Relationship To Other Skills

This skill is the CI/CD specialist in the broader workflow:

- `$ezop-agent-ready-docs` prepares the documentation foundation; this skill ensures pipeline docs within that foundation are accurate
- `$ezop-delivery-planner` may produce slices that involve pipeline changes; this skill verifies those changes are correct and secure
- `$ezop-backlog-loop-orchestrator` may execute slices that modify pipelines; this skill can audit the result
- `$ezop-repo-drift-auditor` detects drift broadly; this skill provides deep CI/CD-specific drift analysis and remediation
- `$ezop-slice-reviewer` reviews completed slices; this skill provides pipeline-specific review expertise
- `$ezop-security-scanner` handles broad security analysis; this skill focuses on pipeline and supply chain security specifically
