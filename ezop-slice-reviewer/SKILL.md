...
1. `$ezop-agent-ready-docs` makes repo guidance trustworthy
2. `$ezop-delivery-planner` produces bounded slices or feature plans
...
```text
$ezop-slice-reviewer zrob review tego jednego bounded slice'a przed commitem.
...
```text
$ezop-slice-reviewer review this single bounded slice before commit.
...
```text
$ezop-slice-reviewer review this slice against the approved plan
```
...
- `$ezop-backlog-loop-orchestrator` has a candidate slice that is almost ready to commit
...
- If the approved slice is missing or too vague, say that review confidence is limited and recommend `$ezop-delivery-planner` first.
- If the surrounding docs are too stale to judge whether docs drift exists, say so and recommend `$ezop-agent-ready-docs`.
