...
- `$ezop-delivery-planner` should be able to trust the repo's doc map, workflow docs, and agent instructions after this skill runs
...
```text
$ezop-agent-ready-docs zbuduj lub odswiez dokumentacje tego repo tak, aby bylo maksymalnie latwe do prowadzenia przez agentow developerskich.
...
```text
$ezop-agent-ready-docs create or refresh this repo's AI-native documentation so it is easy for developer agents to manage.
...
```text
$ezop-agent-ready-docs tighten `AGENTS.md` and related workflow docs for AI-native development
```

Recommended paired invocation when preparing a repo for the full workflow:

```text
$ezop-agent-ready-docs przygotuj dokumentacyjny fundament pod `$ezop-delivery-planner` i `$ezop-backlog-loop-orchestrator`.
...
- `$ezop-delivery-planner` should be able to create plans that rely on real doc ownership and real workflow entry points
- `$ezop-backlog-loop-orchestrator` should be able to read the backlog, trust the docs it depends on, and execute one slice at a time with fewer clarifying pauses
- `$ezop-repo-drift-auditor` should be able to confirm later that docs and workflows stayed aligned instead of first having to untangle the doc map
