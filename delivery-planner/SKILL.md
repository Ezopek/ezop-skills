...
- `$agent-ready-docs` when the documentation surface itself needs tightening
- `$backlog-loop-orchestrator` when the resulting backlog should be executed in an autonomous delivery loop
- `$repo-drift-auditor` when the current roadmap or backlog may no longer match the repo

Think of the intended handoff as:

1. `$agent-ready-docs` makes the repo instructions and doc map trustworthy
2. `$delivery-planner` turns that grounded context into roadmap, backlog, or feature-plan artifacts
3. `$backlog-loop-orchestrator` executes those artifacts one bounded slice at a time
4. `$slice-reviewer` can review each finished slice against the approved plan before commit
5. `$repo-drift-auditor` can periodically confirm that the docs and planning artifacts still match repo reality

## Announce At Start

When this skill activates, tell the user:

> Using **delivery-planner** to [specific purpose based on the request].
...
```text
$delivery-planner przygotuj albo zaktualizuj plan wdrozenia dla tego repo.
...
```text
$delivery-planner create or update an implementation plan for this repo.
...
```text
$delivery-planner turn this feature idea into a bounded implementation plan
```

Recommended paired invocation for backlog creation that will later be executed in a loop:

```text
$delivery-planner przygotuj backlog gotowy do wykonania przez `$backlog-loop-orchestrator`.
Jesli dokumentacja repo nie daje jeszcze stabilnego source-of-truth, zatrzymaj sie i powiedz, ze najpierw warto uzyc `$agent-ready-docs`.
```
...
For backlog artifacts intended for `$backlog-loop-orchestrator`, prefer items that already answer:
...
- be good inputs for `$backlog-loop-orchestrator`
...
Use `$agent-ready-docs` first or recommend it explicitly when:
...
Use `$repo-drift-auditor` first or recommend it explicitly when:
...
- produce slices that are easy for `$backlog-loop-orchestrator` to run one at a time
...
- make scope boundaries explicit enough that `$slice-reviewer` can judge whether implementation stayed inside them
...
- or recommend `$agent-ready-docs` if the repo needs a broader documentation cleanup
...
- `$cicd-guardian` — when the plan touches CI/CD pipelines; have the guardian verify pipeline assumptions
- `$security-scanner` — when the plan involves auth, data handling, or infrastructure changes that need security review
